...
for j in range(len(data)):
    ...
    for i in range(len(invol_trials)):
        row = pd.DataFrame([{"subject": "p%s" % (j + 2),
                             ...
                             "trial_number": i + 1,
                             "trial_status": np.abs(invol_trials[i] - baseline)  < error,
                             "error": error}])
...
for subject in subjects[1:]:
    ...
    test_eeg = eeg_data[b_df[b_df.subject == subject].trial_status.values].mean()
    test_binding = b_df[b_df.subject == subject][b_df[b_df.subject == subject].trial_status == True].binding.values.mean()
